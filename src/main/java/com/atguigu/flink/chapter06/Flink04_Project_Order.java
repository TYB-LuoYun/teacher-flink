...
public class Flink04_Project_Order {
    public static void main(String[] args) {
        ...
        orderEventStream
            .connect(txEventStream) //两个流相互独立，后面的算子需要对两个流分别处理
            .keyBy(OrderEvent::getTxId, TxEvent::getTxId)
            .process(new KeyedCoProcessFunction<String, OrderEvent, TxEvent, String>() {  //key  in1  In2 out
                ...
                public void processElement1(OrderEvent value,
                    ...
                    if (txEvent != null) {
                        ...
                        orderEventMap.put(ctx.getCurrentKey(), value);  //对账不成功的则放入map中等待后续对账
                    }
